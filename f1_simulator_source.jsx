...
function runQ1Q2Q3(drivers, teams, track) {
  const score = (d,team,noise) => calcQualBase(d,team,track)+(Math.random()-0.5)*noise;
...
function aggQual(n, drivers, teams, track) {
  ...
  const gridAcc={}, q1Acc={}, q2Acc={};
  drivers.forEach(d=>{gridAcc[d.id]=[];q1Acc[d.id]=[];q2Acc[d.id]=[];});
  for(let i=0;i<n;i++){
    const q=runQ1Q2Q3(drivers,teams,track);
    q.grid.forEach((e,idx)=>gridAcc[e.driver.id].push(idx+1));
    q.q1.forEach(e=>q1Acc[e.driver.id].push(e.q1Pos));
    q.q2.forEach(e=>q2Acc[e.driver.id].push(e.q2Pos));
  }
  const avg=arr=>arr.length?arr.reduce((a,b)=>a+b,0)/arr.length:999;
  const sorted=drivers.map(d=>{
    const team=teams.find(t=>t.id===d.teamId)||teams[teams.length-1];
    return {driver:d,team,score:100-avg(gridAcc[d.id]),avgGrid:avg(gridAcc[d.id])};
  }).sort((a,b)=>a.avgGrid-b.avgGrid);
  const q1=sorted.map((e,i)=>({...e,q1Pos:i+1,elim1:i>=15}));
  const q2=[...sorted.slice(0,15).map((e,i)=>({...e,q2Pos:i+1,elim2:i>=10})),
            ...sorted.slice(15).map((e,i)=>({...e,q2Pos:16+i,elim2:true}))];
  const q3=sorted.slice(0,10).map((e,i)=>({...e,q3Pos:i+1}));
  return {q1,q2,q3,grid:sorted,runsUsed:n,
...
export default function F1Sim(){
  ...
  const doSprint=()=>{
    if(!qual) return;
    setSprint(aggSprint(runAccuracy,qual.grid,teams,track));
    setView("sprint");
  };
  const doRace=()=>{
    if(!qual) return;
...
{"stages":[
  ...
  const doMultiSim=()=>{
    ...
    setTimeout(()=>{
      ...
      setSimResults(results); setSimRunning(false);
    },50);
  };
    ...
    <div style={{background:"#0C0C0F",minHeight:"100vh",fontFamily:"'Rajdhani','Segoe UI',sans-serif",color:"#F0F0F0"}}>
      ...
      <div style={{background:"#111115",borderBottom:"2px solid #E8002D",display:"flex",alignItems:"center",height:58,paddingLeft:14,overflowX:"auto"}}>
        ...
        <div style={{marginLeft:"auto",display:"flex",alignItems:"center",gap:6,paddingRight:14,flexShrink:0}}>
          <span style={{fontSize:11,color:"#778",letterSpacing:1,whiteSpace:"nowrap",marginRight:2}}>ACCURACY</span>
          {[1,5,10,20].map(n=>(
          ...
          <span style={{fontSize:11,color:"#444",marginLeft:6,whiteSpace:"nowrap"}}>{track.flag} {track.name.toUpperCase()}</span>
        </div>
      </div>
                ...
                {champTab==="history"&&(
                  <div style={{maxHeight:620,overflowY:"auto"}}>
                    {[...championship].reverse().map((r)=>(
                      <div key={r.id} style={{background:"#161618",border:"1px solid #1E1E22",borderRadius:4,padding:"14px 16px",marginBottom:6}}>
                        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:4}}>
                        ...
                        {r.sprintWinner&&(
                          <div style={{marginTop:6,padding:"5px 8px",background:"#FFC90610",border:"1px solid #FFC90622",borderLeft:"3px solid #FFC906",borderRadius:"0 3px 3px 0"}}>
                            <div style={{fontSize:12,color:"#FFC906",fontWeight:700,letterSpacing:1.5,marginBottom:2}}>⚡ SPRINT WINNER</div>
