...
function buildQuarterStandings(finalPositions, totalLaps, quarterWeather) {
  ...
  // Base swap windows — wet weather adds extra positions of chaos
  var baseSwap  = [7, 4, 2, 0];
  // Gap scale [min, max] seconds — wider when wet
  ...
  return [0,1,2,3].map(function(qi) {
    ...
    if(qi === 3) {
      return finalPositions.map(function(e,i){
        return {driver:e.driver,team:e.team,pos:i+1,gap:e.gap,dnf:e.dnf,dnfLap:e.dnfLap,gridPos:e.gridPos,weather:qw[qi]};
      });
    }

    // Copy and shuffle within window
    ...
    // Drivers who DNF'd before this lap go to the back
    var active=[], retired=[];
    ...
    return sorted.map(function(e,i){
      ...
      return {driver:e.driver,team:e.team,pos:i+1,gap:isDNF?null:(i===0?0:qg),dnf:isDNF,dnfLap:isDNF?e.dnfLap:null,gridPos:e.gridPos,weather:qw[qi]};
    });
  });
}
